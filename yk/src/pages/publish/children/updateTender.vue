        ...
        <div class="form-box">
            ...
            <form action="" class="form-container layui-form">
                ...
                <div class="form-group">
                    <label for="">标题：</label>
                    <input type="text" v-model:value="order.t_title" placeholder="必填"/>
                </div>
                ...
                <div class="form-group radio-group">
                    ...
                    <p style="display: inline-block;">
                        <input id="type1" 
                        v-bind:checked=" order.t_type=='全新' " 
                        @click="order.t_type='全新' " type="radio" style="opacity:0!important;" name="type-order"/>
                ...
                <div class="form-group">
                    <label for="">数量：</label>
                    <input type="text" v-model:value="order.t_quality"  placeholder="必填"/>
                    <span class="unit" @click="alertSelectBox(3)"  v-text="unit.u_name"></span>
                </div>



<!--                 <div class="form-group radio-group" style="display: flex;align-items:center;">
                    <label for="">货物单价：</label>
                    <p style="display: inline-block;">
                        <input id="tPrice1"
                        v-bind:checked=" order.t_price <= 0 "
                        @click=" openPrice=false " name="openPrice" type="radio"/>
                        <label for="tPrice1" class="type-icon"></label>不开价
                    </p>
                    <p style="display:inline-block;">
                        <input id="tPrice2" v-bind:checked=" order.t_price > 0 "
                        @click=" openPrice=true " name="openPrice" type="radio" />
                        <label for="tPrice2" class="type-icon"></label>开价
                    </p>
                    <p v-if="openPrice" >
                        <input type="number" lay-verify="tPrice" v-bind:value="order.t_price" style="padding-left:8px;" placeholder="请输入价格"/>
                    </p>
                </div> -->


                <div class="form-group radio-group price-group">
                    <p><label for="">货物单价：</label></p>
                    <p>
                        <!-- <input id="tPrice1" lay-ignore @click=" openPrice=false " checked name="openPrice" type="radio"/> -->
                        <input id="tPrice1"
                        v-bind:checked=" order.t_price <= 0 "
                        @click=" openPrice = false " name="openPrice" type="radio"/>
                    ...
                    <p>
                        <span v-if="openPrice" >
                          <input class="tender_price" type="number" v-model:value=" order.t_price " placeholder="请输入价格"/>
                        </span>
                    </p>
                </div>
                ...
                <div class="form-group">
                    <label for="">联系人：</label>
                    <input type="text" v-model:value="order.name" placeholder="必填"/>
                </div>
                <div class="form-group">
                    <label for="">联系电话：</label>
                    <input type="text" v-model:value="order.phone" placeholder="必填"/>
                </div>
                <div class="form-group">
                    <label for="">交易地址：</label>
                    <input type="text" v-model:value="order.address" placeholder="必填"/>
                </div>
                <div class="form-group">
                    <label for="">公司名称：</label>
                    <!--<input type="text" v-if="order.company == 'undefined' "  v-model:value="" placeholder=""/>-->
                    <input type="text" v-model:value="order.company" placeholder=""/>
                </div>
                <div class="form-group">
                    <label for="">邮箱：</label>
                    <input type="text" v-model:value="order.email" placeholder=""/>
                </div>

                <div class="form-group icon-gt">
                    <label for="">交易结束时间：</label>
                    <input type="text" readonly lay-verify="endDate" @click="alertSelectBox(5)"  v-model:value="order.t_date" placeholder="必填"/>
                </div>
                ...
                <div class="form-group desc-group">
                    ...
                    <textarea v-model:value="order.t_detail"></textarea>
                </div>
                <div class="form-group image-group main-img" id="upload1">
                    ...
                    <!-- 上传图片组件框 -->
                    <drop-zone id="myVueDropzone" class="myVueDropzone5" ref='uploadImg' :useFontAwesome="false" enctype="multipart/form-data" methods='post' 
                    ...
                    <div v-show=" uploadSwitch " class="img-upload-box dz-default dz-message">
                        <img src="../../../assets/imgs/publish/img-upMain.png" alt="" />
                    </div>
					...
					<div class="dz-preview dz-image-preview" v-for=" (item,index) in pFileArr ">
						...
						<div class="dz-desc">
						  <textarea class="priList" placeholder="请为你的图片添加说明，例如铜含量60%" :value="priList[index]" maxlength="50" name="priList"></textarea>
						</div>
                ...
                <div id="upload2" class="form-group image-group info-img">
                    ...
                    <!-- 上传图片组件框 -->
                    <drop-zone id="myVueDropzone2" class="myVueDropzone5" ref='uploadImg2' :useFontAwesome="true" enctype="multipart/form-data" methods='post' 
                      :autoProcessQueue=false
                      :maxFileSizeInMB=10
                      :showRemoveLink=true
					...
					<div class="dz-preview dz-image-preview" v-for=" (item,index) in pDetailArr ">
						...
						<div class="dz-desc">
						  <textarea class="priList" placeholder="请为你的图片添加说明，例如铜含量60%" :value="detList[index]" maxlength="50" name="priList"></textarea>
						</div>
                ...
                <div class="form-group agree-group">
                    ...
                    <span @click="showRule">我已阅读<i>《再商网现货交易细则》</i></span>
                </div>
            </form>
        </div>
...
import {showRule1,uploadUrl} from '@/tool/tool'
...
export default{
      ...
      this.$nextTick(()=>{
        ...
        setTimeout(()=>{
          ...
          this.scroller = new bScroll(".fbcg",{
            ...
            // swipeTime:100,
            // momentumLimitTime: 2300, //符合惯性拖动的最大时间
            // momentumLimitDistance: 50, //符合惯性拖动的最小拖动距离
          })
  ...
  watch:{
    ...
    endTime:function(val){
        this.order.t_date = val;
        this.showSelectBox = false;
    },
    getTime:function(val){
        this.order.t_picktime = val;
    ...
    currency:function(val){
        ...
        this.order.currency_id = val.c_id;
    ...
    unit:function(val){
        ...
        this.order.unit_id = val.u_id;
        this.showSelectBox = false;

    },
    //交易方式
    tradeType:function(val){
        this.order.trade_id = val.t_id;
  ...
  methods:{
  		...
  		this.$http.post('/queryTenderByid',this.$route.query).then((res)=>{
  			...
  			if(res.data.code == 0){
                ...
                console.log(this.newDetList);
  				...
  				//主图描述
  				this.priList = JSON.parse(order.priList)
  				//详情图描述
  				this.detList = JSON.parse(order.detList)
  				//判断是否开价
    ...
    complete(){
      ...
      if( this.$refs.uploadImg.getUploadingFiles().length==0 && this.$refs.uploadImg2.getUploadingFiles().length ==0 ){
        //priList0表示历史记录，所有这里依旧使用priList. 图片描述信息
      ...
      return `
              <div class="dz-preview dz-file-preview">
                ...
                <div class="dz-desc">
                  ...
                  <textarea class="priList" placeholder="请为你的图片添加说明，例如铜含量60%" maxlength="50" name="priList"></textarea>
...
//        time:10,
    ...
    submitForm(){
      ...
      //更新图片描述信息
      this.order.priList = JSON.stringify(this.priList);
      this.order.detList = JSON.stringify(this.detList);

      //更新图片路径
      this.order.p_file = this.newPriList.toString();
      this.order.p_detail = this.newDetList.toString();

      console.log(this.order);
      let currentOrder = this.order;
      //复制新订单对象，符合后台设置的字段
      let newTender = {
          bid:currentOrder.t_id,
          tTitle:currentOrder.t_title,
          tType:currentOrder.t_type,
          tTendertype:currentOrder.t_tendertype,
          tQuality:currentOrder.t_quality,
          tDate:currentOrder.t_date,
          tPicktime:currentOrder.t_picktime,
          tDetail:currentOrder.t_detail,
          areaId:currentOrder.area_id,
          pId:currentOrder.p_id,
          usrId:currentOrder.usr_id,
          unitId:currentOrder.unit_id,
          tradeId:currentOrder.trade_id,
          packId:currentOrder.pack_id,
          currencyId:currentOrder.currency_id,
          pFile:currentOrder.p_file,
          pDetail:currentOrder.p_detail,

          pCargolist:'',
          dw_id:currentOrder.dw_id,
          pRar:'',
          listRar:'',
          tPrice:currentOrder.t_price,
          cName:currentOrder.name,
          company:currentOrder.company,
          email:currentOrder.email,
          phone:currentOrder.phone,
          address:currentOrder.address,

          priList:currentOrder.priList,
          detList:currentOrder.detList,
      ...
      //修改订单
      this.$http.post('/ModifyTender',newTender).then((res)=>{
      	console.log(222222)
        //后台传来结果，关闭加载动画
        layer.closeAll();
        if(res.data.code==0){
    ...
    selectProductType(item){
      ...
      this.order.p_id = item.getAttribute("data-id");

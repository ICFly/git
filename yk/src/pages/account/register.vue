...
<div class="register_page">
    ...
    <div class="header">
        ...
        <div class="header-type">
            <span @click="typeReg=1" v-bind:class="{active:typeReg==1}">手机注册</span>
            <span @click="typeReg=2" v-bind:class="{active:typeReg==2}">邮箱注册</span>
        </div>
    </div>

    <div class="content" style="height:100%;overflow:hidden;">
        <form action="" class="form-box layui-form" id="layui-form" style="height:100%;overflow:hidden;">

            <div class="form-group" v-if="typeReg==1">
                <span class="area-code" @click='areaBox=!areaBox' 
                :class="{active:currentArea.text}"  v-text=" currentArea.text ||'选择区号' "></span>
                <input type="text" name='mobile' placeholder="请输入手机号" v-model:value="user.phone" class="pd form-control">
            </div>
            ...
            <div class="form-group">
                <input type="text" placeholder="请输入邀请码(非必填)" v-model:value="user.referrer_vip" class="form-control">
            </div>
            <div class="form-group">
                <input type="number" maxlength="6" placeholder="请输入验证码" v-model:value="user.code" class="form-control">
                <span class="btn btn-getCode on" v-if='sendCode' @click='getCode'>获取验证码</span>
                <span class="btn btn-getCode" v-else >{{timer}}秒</span>
            </div>
            <div class="btn-group">
                <button class="btn btn-register" @click.prevent='submit'>注册</button>
            </div>
        </form>
    </div>
    ...
    submit(){
      ...
      this.$http.post(url,options).then((res)=>{
        if(res.data.code==0){
          ...
          // this.$router.push({path:'/login',query:{user:user.phone}})
          let vm = this;
          layer.msg("注册成功",{time:1.5},()=>{
            vm.$router.push({path:'/login',query:{user:options.name}});
            //重置表单
            vm.reset();
          })
